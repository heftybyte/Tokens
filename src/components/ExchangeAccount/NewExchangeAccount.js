...
import { addExchangeAccount } from '../../reducers/account'
...
class NewExchangeAccount extends Component {
    ...
    submit =  async () => {
        const { navigation, setLoading, showToast, addExchangeAccount } = this.props
        const { platform } = navigation.state.params
        const { key, secret, name, passphrase } = this.state
        console.log({ key, secret, name, passphrase, platform })
        try {
            await addExchangeAccount({ key, secret, name, passphrase, platform })
            setLoading(false)
        } catch (err) {
            setLoading(false)
            console.error(err)
    ...
    render() {
        const { navigation, exchangeMap  } = this.props
        const { platform } = navigation.state.params
        const exchange = exchangeMap[platform]
            ...
            <StyleProvider style={getTheme(_platform)}>
                <Container>
                    <Content>
                        ...
                        {exchange.requiresPassphrase && <View style={[customStyles.header, {flex: 1}]}>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        addExchangeAccount: ({ key, secret, name, passphrase, platform }) =>
            dispatch(addExchangeAccount({ key, secret, name, passphrase, platform }))
    }
}
